...
Sub Main
	...
	'Call exportCPHfield(Nports, Freq,xmin,xmax,ymin,ymax,zmin,zmax,stepsize)
End Sub
...
'# Individual Channels
For i=1 To Nports
	If SelectTreeItem ("2D/3D Results\H-Field\h-field (f=" & Freq & ") [CH" & i & "]") Then
	    Plot3DPlotsOn2DPlane False
	    Wait(1)
		With ASCIIExport
			.Reset
...
Sub exportCPHfield(Nports, Freq,xmin,xmax,ymin,ymax,zmin,zmax,stepsize)
Dim i, j As Double
Dim sExportFolder As String
'Dim stepsize As Double
'stepsize = 2
...
If SelectTreeItem ("2D/3D Results\H-Field\h-field (f=" & Freq & ") [CP_Mode]") Then
	Plot3DPlotsOn2DPlane False
	Wait(1)
	With ASCIIExport
		.Reset
		.FileName (sExportFolder + ".\H_CP_Mode.h5")
		.Mode ("FixedWidth")
		.SetfileType("hdf5")
		.SetSubvolume(xmin,xmax,ymin,ymax,zmin,zmax)
		.UseSubvolume(True)
